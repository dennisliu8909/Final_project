THRESHOLD = (0,90) # Grayscale threshold for dark things...
...
while(True):
    ...
    if (img.find_apriltags()):
        ...
        BINARY_VISIBLE = True
        img = img.binary([THRESHOLD]) if BINARY_VISIBLE else sensor.snapshot()
        line = img.get_regression([(255,255) if BINARY_VISIBLE else THRESHOLD], False, (5, 0, 150, 20))
        ...
        if (line) :
            ...
            if (abs(diff_dis) <= 20 and diff_dis < 0 and theta > 60) :  # turn right
                uart.write(("/turn2/run 70 -0.35 \n").encode())
                print("right")
            elif(abs(diff_dis) <= 20 and diff_dis > 0 and theta > 60) : # turn left
                uart.write(("/turn2/run 70 0.35 \n").encode())
                print("left")
